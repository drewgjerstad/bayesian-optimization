...
non-convex. The left plot is convex and has a unique optimum, the middle plot is
...
Since the plot on the left is convex, we can apply gradient-based methods and,
in conjunction with a reasonable learning rate, locate the unique optimum.
However, the middle and right-side plots are much more difficult. Consider the
middle plot, for example. Using gradient-based methods runs the risk that we
become "trapped" in one of the local optima and not reach the global optimum
(global minimum: $x\approx 5.75$, global maximum: $x\approx 2.5$). Another
troubling case is the one depicted in the plot on the right: saddle points.
Saddle points are problematic for classical methods since they form a flat
region prohibiting the progression of the optimizer towards the optimum.

Additionally, while the two plots to the right show non-convex functions which
are problematic to optimize, there are other characteristics of objective
functions that can lead to issues and are typically addressed via the Bayesian
optimization approach. Such characteristics include:
